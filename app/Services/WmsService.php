            ...
            'warehouses' => $warehouses->map(fn (WarehouseZone $warehouse) => [
                ...
                'blocks' => $warehouse->racks->map(fn (WarehouseRack $rack) => [
                    ...
                    'inventories' => $rack->inventories->map(fn (PipeInventory $inventory) => [
                        ...
                        'mill_source' => $inventory->mill_source ?? 'PT SPINDO Tbk Unit 7 Gresik',
                        'qty_bundles' => (int) $inventory->qty_bundles,
                        'qty_pcs' => (int) $inventory->qty_pcs,
                        'total_weight_kg' => (float) $inventory->total_weight_kg,
                        'status' => $inventory->status,
                        'qc_status' => $inventory->qc_status,
                        'inbound_date' => $inventory->inbound_date ? (is_string($inventory->inbound_date) ? substr($inventory->inbound_date, 0, 10) : $inventory->inbound_date->format('Y-m-d')) : null,
                        'description' => $inventory->description ?: $inventory->product?->description,
                        'product' => $inventory->product ? [
                            'id' => $inventory->product->id,
                            'sap_code' => $inventory->product->sap_code,
                            'nama_mudah' => $inventory->product->nama_mudah,
                            'description' => $inventory->product->description,
                            'jenis' => $inventory->product->jenis,
                            'nominal_size' => $inventory->product->nominal_size,
                            'spec_name' => $inventory->product->spec_name,
                            'category' => $inventory->product->category?->name,
                            'category_code' => $inventory->product->category?->code,
                            'outer_diameter_mm' => (float) ($inventory->product->outer_diameter_mm ?? 0),
                            'length_meters' => (float) ($inventory->product->length_meters ?? 6.00),
                            'is_threaded' => (bool) $inventory->product->is_threaded,
                            'pcs_per_bundle' => (int) $inventory->product->pcs_per_bundle,
                            'weight_per_bundle_kg' => (float) ($inventory->product->weight_per_bundle_kg ?? 0),
                        ] : null,
                    ]),
                ]),
            ]),
